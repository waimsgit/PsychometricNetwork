				...
				name1:function(){
					...
					if(Tsval==""){
						...
						if(nameIsserver[0]==1){
							...
							$.ajax({
								...
								success:function(data){
									if(data==1){
										...
										$this.siblings("p").html("用户名已被注册").removeAttr("class").css({"color":"red"});
									}
								},
								error:function(xml,error){
									console.log(error);
							...
							$(this).siblings("p").html("输入正确").attr({"class":"correct"}).css({"color":"green"});
						}
					}
				},
				//密码验证
				password:function(){
					...
					if(Tsval==""){
						...
						if(PwdStrong[0]==1){
							...
							$(this).siblings("p").attr({"class":"correct"}).css({"color":"green"}).html("密码正确");
						}
					}
					...
					if($(this).val()!=mypasswords.val()&&mypasswords.val()!=""){
						...
						mypasswords.siblings("p").attr({"class":"correct"}).css({"color":"green"}).html("密码一致");
					}
				},
				//确认密码验证
				passwords:function(){
					...
					if(Tsval==""){
						...
						$(this).siblings("p").attr({"class":"correct"}).css({"color":"green"}).html("密码一致");
					}
				},
				//身份证验证
				card:function(){
				...
				mailbox:function(){
					...
					if($(this).val()==""){
						...
						if(mailboxIsServer[0]==1){
							...
							$(this).siblings("p").attr({"class":"correct"}).css({"color":"green"}).html("邮箱正确");
						}
					}
				},
				//验证码验证
